...
## Activated local checkout

Private material is in ignored `.runtime/active`, with original input and the
client CA key in `.runtime/input`. `deploy-local.sh lt` includes this overlay only
when `.runtime/enabled` exists; preparation alone does not enable it. Remove that
