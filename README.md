...
converter. Unlike historical recovery, guarded graph, Role, and Rule events use
the normal command append path. Category and Tag births use a narrow direct path,
then the importer waits for their projections and creates the required identity
materialization rows before it reports success. Guarded bootstrap events retain
normal append validation and graph stamping, while their projection-only recovery
copy skips the unavailable Light Gateway binary compiler for `mcp-router.tools`.
